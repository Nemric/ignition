...
func TestSystemdUnitPath(t *testing.T) {
	...
	for i, test := range tests {
		path := st.SystemdUnitPaths(test.in)
		if path[len(path)-1] != test.out[len(test.out)-1] {
...
func TestSystemdDropinsPaths(t *testing.T) {
	...
	for i, test := range tests {
		path := st.SystemdDropinsPaths(test.in)
		if path[len(path)-1] != test.out[len(test.out)-1] {
			t.Errorf("#%d: bad error: want %v, got %v", i, test.out, path)
		}
	}
...
func TestCreateUnits(t *testing.T) {
	...
	config, report, err := v3_4_experimental.Parse([]byte(conf))
